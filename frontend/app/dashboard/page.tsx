...
export default function Dashboard() {
  ...
  async function file(e: any) {
    ...
    setErr('');
    try {
      setResume(await uploadResume(f));
    } catch (x: any) {
      setErr(x.message);
    } finally {
      setBusy(false);
    ...
    try {
      ...
      setErr(
        result.imported || result.updated
          ? `Synced ${result.imported} new and ${result.updated} updated jobs.`
          : 'No new jobs found.'
      );
    } catch (x: any) {
      setErr(x.message);
  ...
  async function run() {
    if (!resume) {
      setErr('Upload your resume first.');
      return;
    }
    setBusy(true);
    setErr('');
    try {
      let jid = job ? Number(job) : undefined;
      if (jd.trim()) {
      ...
      const result = await analyze(resume.id, jid);
      sessionStorage.setItem('rv_last_analysis', JSON.stringify(result));
    ...
    <AppShell>
      <section className="welcome">
        <div>
          ...
          <p>Upload your resume and get AI-powered insights with real job market evidence.</p>
        </div>
      ...
      <section id="upload" className="panel uploadPanel">
        <div className="drop">
          ...
          <p>
            {resume
              ? `${resume.parsed?.skills?.length || 0} technologies detected`
              : 'Drag & drop your file here, or click to browse'}
          </p>
          <small>Supports PDF, DOCX, TXT (Max 10MB)</small>
          <label className="primary fileBtn">
            {busy ? 'Processing…' : 'Choose File'}
            <input hidden type="file" accept=".pdf,.docx,.txt" onChange={file} />
          </label>
          {resume && (
            <div className="skillPreview">
              {resume.parsed?.skills?.slice(0, 8).map((x: string) => (
                <span key={x}>{x}</span>
              ))}
            </div>
          )}
        ...
        <div className="jobBox">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
            <label style={{ flex: 1 }}>
              Choose an archived job posting
              <select value={job} onChange={e => setJob(e.target.value)}>
                <option value="">Evidence scan only</option>
                {jobs.map(j => (
                  <option key={j.id} value={j.id}>{j.company} — {j.title}</option>
                ))}
              </select>
            </label>
            <button type="button" className="secondary" onClick={refreshData} disabled={busy} style={{ whiteSpace: 'nowrap' }}>
              Refresh jobs
            </button>
          </div>
          ...
          <div className="analyzeRow">
            ...
            />
            <button className="primary analyzeBtn" onClick={run} disabled={busy}>
